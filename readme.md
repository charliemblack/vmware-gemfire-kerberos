...
## Installing Kerbros
```shell
sudo apt-get install krb5-kdc krb5-admin-server krb5-config -y
```

Add the realm:
![Add the default realm](docs/kerberos-default-realm.png)

Add the servers:
![Servers for the Realm](docs/servers-for-realm.png)

Add the admin server:
![admin server](docs/admin-server.png)

Initalize the realm and enter the password for the admin user:
```shell
sudo krb5_newrealm
```

Allow the admin user to login.   This done by adding `*/admin *` to the following file:
```shell
...
```
root@demo-virtual-machine:/home/demo# kadmin.local 
Authenticating as principal root/admin@DEMO-VIRTUAL-MACHINE with password.
kadmin.local:  addprinc -randkey app/demo-virtual-machine
WARNING: no policy specified for app/demo-virtual-machine@DEMO-VIRTUAL-MACHINE; defaulting to no policy
Principal "app/demo-virtual-machine@DEMO-VIRTUAL-MACHINE" created.
kadmin.local:  
...
kadmin.local:  ktadd -k gemfire_app.keytab gemfire/demo-virtual-machine
kadmin.local: Principal gemfire/demo-virtual-machine does not exist.
kadmin.local:  addprinc -randkey gemfire/demo-virtual-machine
WARNING: no policy specified for gemfire/demo-virtual-machine@DEMO-VIRTUAL-MACHINE; defaulting to no policy
Principal "gemfire/demo-virtual-machine@DEMO-VIRTUAL-MACHINE" created.
kadmin.local:  ktadd -k gemfire_app.keytab gemfire/demo-virtual-machine
Entry for principal gemfire/demo-virtual-machine with kvno 2, encryption type aes256-cts-hmac-sha1-96 added to keytab WRFILE:gemfire_app.keytab.
Entry for principal gemfire/demo-virtual-machine with kvno 2, encryption type aes128-cts-hmac-sha1-96 added to keytab WRFILE:gemfire_app.keytab.
kadmin.local:  quit
```
